...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const coverColor = async () => {
    ...
    if (!path) {
      // 非文章情况，直接设置不需要请求了
      root.style.setProperty("--icemyst-bar-background", "var(--icemyst-meta-theme-color)");
      btf.initThemeColor();

      // 要改回来默认主色
      document.documentElement.style.setProperty(
        "--icemyst-main",
        getComputedStyle(document.documentElement).getPropertyValue("--icemyst-theme")
      );
      document.documentElement.style.setProperty(
        "--icemyst-theme-op",
        getComputedStyle(document.documentElement).getPropertyValue("--icemyst-main") + "23"
      );
      document.documentElement.style.setProperty(
        "--icemyst-theme-op-deep",
        getComputedStyle(document.documentElement).getPropertyValue("--icemyst-main") + "dd"
      );
    ...
    if (GLOBAL_CONFIG.mainTone) {
      if (GLOBAL_CONFIG_SITE.postMainColor) {
        ...
        root.style.setProperty("--icemyst-bar-background", value);
        btf.initThemeColor();

        if (GLOBAL_CONFIG.mainTone.cover_change) {
          document.documentElement.style.setProperty("--icemyst-main", value);
          document.documentElement.style.setProperty(
            "--icemyst-theme-op",
            getComputedStyle(document.documentElement).getPropertyValue("--icemyst-main") + "23"
          );
        ...
        const fallbackValue = "var(--icemyst-theme)";
        let fetchPath = "";
            ...
            if (GLOBAL_CONFIG.mainTone.mode == "both") {
              ...
              try {
                ...
                if (response.ok && response.headers.get("content-type")?.includes("application/json")) {
                  ...
                  root.style.setProperty("--icemyst-bar-background", fallbackValue);
                  btf.initThemeColor();
                  document.documentElement.style.setProperty("--icemyst-main", fallbackValue);
                }
              } catch {
                root.style.setProperty("--icemyst-bar-background", fallbackValue);
                btf.initThemeColor();
